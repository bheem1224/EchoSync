...
from database.music_database import Artist, AudioFingerprint, Track, get_database
...
class DuplicateHygieneService:
    ...
    def find_duplicates(self) -> dict[str, list[dict]]:
        ...
        """
        results = {"auto_resolve": [], "manual_review": []}
        ...
        try:
            with self.db.session_scope() as session:
                ...
                for fp_hash in duplicate_hashes:
                    ...
                    track_ids = list(
                        {fp.media.track_id for fp in fingerprints if fp.media}
                    )
    ...
    def _analyze_group(self, tracks: list[Track], chromaprint: str) -> dict | None:
        ...
        for track in tracks:
            if not track.file_path or not os.path.exists(track.file_path):
                logger.warning(
                    f"JIT Check Failed: Track {track.id} file missing at {track.file_path}"
                )
                continue
            ...
            try:
                ...
                logger.error(
                    f"JIT Check Failed: Could not generate fingerprint for {track.file_path}: {e}"
                )
                continue

            if live_chromaprint != chromaprint:
                logger.warning(
                    f"JIT Check Failed: Track {track.id} live fingerprint does not match database"
                )
                continue
        ...
        def sort_key(t):
            ...
            file_size = max(
                [m.file_size_bytes for m in t.media if m.file_size_bytes] or [0]
            )
            effective_br = br if br <= max_bitrate else -br
            return (effective_br, sample_rate, file_size)
