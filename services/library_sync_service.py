...
class LibrarySyncService:
    ...
    def sync_library(self, scan_mode: str = "incremental"):
        ...
        with acquire_library_lock(task_name="library_sync", blocking=False) as acquired:
            ...
            def parse_file(path: str):
                ...
                try:
                    if hasattr(echosync_core, "read_metadata"):
                        return echosync_core.read_metadata(path), path
            ...
            def _is_generic_or_empty(s: str | None) -> bool:
                if not s:
                    return True
                clean = str(s).strip().lower()
                if not clean or clean in {
                    "unknown", "unknown artist", "unknown title", "untitled",
                    "track", "audio", "music", "song", "none", "null",
                    "corrupted_untagged", "corrupted", "sample", "temp", "tmp"
                }:
                    return True
                if re.match(r"^[\d\s\-_.]+$", clean):
                    return True
                if re.match(r"^(track|audio|sound|music|song|sample|temp|tmp|file|corrupted)[\s\-_.]*\d*$", clean):
                    return True
                return False
            ...
            with ThreadPoolExecutor(max_workers=min(2, os.cpu_count() or 1)) as executor:
                for raw_dict, file_path in executor.map(parse_file, dirty_or_new):
                    ...
                    is_unidentifiable = (not has_embedded_title and not has_embedded_artist) and (not has_inferred_title and not has_inferred_artist)

                    if is_unidentifiable and os.path.exists(file_path):
                        logger.warning(f"Unidentifiable media file detected: '{file_path}'. Ejecting to quarantine: '{quarantine_dir}'.")
                        try:
                            ...
                            if os.path.exists(dest_path) and os.path.abspath(dest_path) != os.path.abspath(file_path):
                                ...
                                dest_path = os.path.join(quarantine_dir, f"{base}_{int(time.time())}{ext}")
                            shutil.move(file_path, dest_path)
                            logger.info(f"Moved unidentifiable file to quarantine: '{file_path}' -> '{dest_path}'")
            ...
            if ejected_files_count > 0:
                logger.info(f"Ejected {ejected_files_count} unidentifiable file(s) to quarantine directory: {quarantine_dir}")
