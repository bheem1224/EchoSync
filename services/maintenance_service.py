...
def run_media_decoupling_job(
    ...
    logger.info(f"Starting media decoupling job [{reg_id}] with threshold {duration_threshold_ms}ms")
    ...
    try:
        ...
        logger.info(f"Media decoupling completed [{reg_id}]: {decoupled_count} track(s) decoupled")
        if progress_callback:
            try:
                progress_callback(100, 100, f"Completed: {decoupled_count} track(s) decoupled")
            except Exception:
                pass
        ...
        logger.error(f"Error during media decoupling job [{reg_id}]: {e}", exc_info=True)
        if progress_callback:
            try:
                progress_callback(0, 100, f"Failed: {str(e)}")
            except Exception:
                pass
        raise
    finally:
