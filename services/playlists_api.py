...
from database.music_database import Album, Artist, LocalMedia, Track, TrackArtist
...
def check_cover_rejection(
    source_title: str, source_artist: str, candidate_diagnostics: list[dict]
) -> bool:
    """
    ...
    for cand_diag in candidate_diagnostics:
        ...
        t_sim = (
            SequenceMatcher(None, src_norm, cand_norm).ratio()
            if (src_norm and cand_norm)
            else 0.0
        )
        if t_sim < 0.90:
...
def get_library_candidates(
    session: Session, target_title: str, target_artist: str
) -> list[Track]:
    """
    ...
    all_artists = (
        ([primary_art] + collabs)
        if primary_art
        else ([target_artist] if target_artist else [])
    )
    ...
    query = (
        ...
        .outerjoin(
            Artist,
            or_(Track.artist_id == Artist.id, TrackArtist.artist_id == Artist.id),
        )
        ...
        if tid not in claimed:
            ...
            for fallback in track.get("candidate_matches", []):
                ...
                if f_id and f_id not in claimed and fallback.get("score", 0) >= 70:
                    ...
                    track["library_match"] = (
                        "Found"
                        if fallback["score"] >= 85
                        else f"Found (score: {int(fallback['score'])}%)"
                    )
                    claimed[f_id] = track
                    fallback_found = True
                    break
            ...
            if not fallback_found:
                ...
                track["rejection_reason"] = (
                    f"Collision: local track assigned to higher confidence match ({winner_artist})"
                )

    return all_tracks


def filter_unevaluated_candidates(
    candidates: list, evaluated_candidate_ids: set
) -> list:
    ...
    for cand in candidates:
        cand_id = (
            cand.id
            if hasattr(cand, "id")
            else (cand[0] if isinstance(cand, (tuple, list)) else cand.get("id"))
        )
...
def check_title_recovery(
    source_artist: str, source_title: str, candidate_artist: str, candidate_title: str
) -> bool:
    """
    ...
    if cand_art_clean in {
        ...
        if match:
            ...
            if (
                _cmp_artists(source_artist, extracted_artist) >= 0.85
                and _cmp_titles(source_title, extracted_title) >= 0.85
            ):
...
def evaluate_tier2_candidate(
    ...
    if check_title_recovery(
        ...
        elif not candidate_track.artist_name or cand_art_clean in {
            "unknown artist",
            "unknown",
            "various artists",
            "various",
