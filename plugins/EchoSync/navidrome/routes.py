...
def get_settings():
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Failed to get Navidrome settings"}, status_code=500
        )
...
async def save_settings(request: Request):
    ...
    if PluginRegistry.is_plugin_disabled("navidrome"):
        return JSONResponse(
            content={"error": "Navidrome provider disabled"}, status_code=403
        )
    try:
        ...
        if "password" in data:
            ...
            logger.info("Navidrome password saved")

        return {"success": True}
    except Exception as e:
        logger.error(f"Error saving Navidrome settings: {e}", exc_info=True)
        return JSONResponse(
            content={"error": "Failed to save Navidrome settings"}, status_code=500
        )
...
def activate_server():
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Failed to activate Navidrome server"}, status_code=500
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if subsonic_response.get("status") == "ok":
                ...
                error_msg = subsonic_response.get("error", {}).get(
                    "message", "Authentication failed"
                )
                return JSONResponse(
                    content={"connected": False, "error": error_msg}, status_code=400
                )
        else:
            return JSONResponse(
