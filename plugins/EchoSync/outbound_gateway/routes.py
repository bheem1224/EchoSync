...
def require_api_key():
    ...
    raise HTTPException(
        status_code=401,
        detail="Unauthorized: API Key invalid or missing (Gateway Auth Not Implemented)",
    )


@router.api_route(
    "/{plugin_id}/{subpath:path}",
...
def gateway_proxy(plugin_id: str, subpath: str):
    ...
    if not ExternalGatewayRegistry.is_registered(plugin_id, subpath):
        raise HTTPException(
            status_code=404, detail="Route not found or not exposed externally"
        )
