...
def get_settings():
    ...
    try:
        ...
        if base_url and username and password:
            try:
                ...
                response = requests.post(
                    auth_url, json=auth_data, headers=headers, timeout=5
                )
        ...
        return JSONResponse(
            content={"error": "Failed to get Jellyfin settings"}, status_code=500
        )
...
async def save_settings(request: Request):
    ...
    if PluginRegistry.is_plugin_disabled("jellyfin"):
        return JSONResponse(
            content={"error": "Jellyfin provider disabled"}, status_code=403
        )
    try:
        ...
        if "password" in data:
            ...
            logger.info("Jellyfin password saved")

        return {"success": True}
    except Exception as e:
        logger.error(f"Error saving Jellyfin settings: {e}", exc_info=True)
        return JSONResponse(
            content={"error": "Failed to save Jellyfin settings"}, status_code=500
        )
...
def activate_server():
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Failed to activate Jellyfin server"}, status_code=500
        )
...
def test_connection():
    ...
    try:
        ...
        if not base_url:
            return JSONResponse(
                content={"connected": False, "error": "Server URL is required"},
                status_code=400,
            )

        if not username:
            return JSONResponse(
                content={"connected": False, "error": "Username is required"},
                status_code=400,
            )
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"Jellyfin connection successful: {server_name} version {version}"
            )
            ...
            return JSONResponse(
                content={"connected": False, "error": "Invalid username or password"},
                status_code=400,
            )
        else:
            return JSONResponse(
                content={"connected": False, "error": f"HTTP {response.status_code}"},
                status_code=400,
            )

    except ImportError:
        return JSONResponse(
            content={"error": "requests library not available"}, status_code=500
        )
    except Exception as e:
        logger.error(f"Jellyfin connection test failed: {e}", exc_info=True)
        return JSONResponse(
            content={"connected": False, "error": "Connection test failed"},
            status_code=400,
        )
