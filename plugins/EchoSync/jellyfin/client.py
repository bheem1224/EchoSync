...
class JellyfinAlbum:
    ...
    def artist(self) -> JellyfinArtist | None:
...
class JellyfinTrack:
    ...
    def album(self) -> JellyfinAlbum | None:
...
class JellyfinClient(MediaServerProvider):
    ...
    def create_playlist(self, name: str, tracks: list) -> bool:
        ...
        track_ids = [
            _safe_getattr(t, "ratingKey", None)
            for t in tracks
        ...
        data = {"Name": name, "UserId": self.user_id, "MediaType": "Audio"}
    ...
    def __init__(self):
        ...
        self.music_library_id: str | None = None
        ...
        def jellyfin_health_check() -> HealthCheckResult:
            try:
                ...
                return HealthCheckResult(
                    service_name="jellyfin",
                    status="unhealthy",
                    message=f"Jellyfin connection error: {e!s}",
                )
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            and self.music_library_id is not None
        )
    ...
    def get_all_artists(self) -> list[JellyfinArtist]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(artists)} album artists from Jellyfin AlbumArtists endpoint (with aggressive caching)"
            )
            return artists
    ...
    def get_tracks_for_album(self, album_id: str) -> list[JellyfinTrack]:
        ...
        try:
            ...
            fields = [
                ...
                "Bitrate",
                "MediaSources",
                "MediaStreams",  # Audio quality fields
    ...
    def get_artist_by_id(self, artist_id: str) -> JellyfinArtist | None:
    ...
    def get_album_by_id(self, album_id: str) -> JellyfinAlbum | None:
    ...
    def get_playlist_by_name(self, name: str) -> JellyfinPlaylistInfo | None:
        ...
        for playlist in playlists:
            ...
            logger.info(
                f"Creating Jellyfin/Emby playlist '{name}' with {len(track_ids)} valid track IDs (filtered {len(invalid_tracks)} invalid)"
            )
        ...
        try:
            ...
            if existing_playlist:
                ...
                if response.status_code in [200, 204]:
                    ...
                    logger.warning(
                        f"Could not delete existing playlist '{playlist_name}' (status: {response.status_code}), creating anyway"
                    )
    ...
    def add_tracks_to_playlist(
        ...
        try:
            ...
            add_params = {"Ids": ",".join(valid_ids), "UserId": self.user_id}
    ...
    def remove_tracks_from_playlist(
        ...
        try:
            ...
            remaining_ids = [
                ...
                if str(_safe_getattr(track, "ratingKey", ""))
                and str(_safe_getattr(track, "ratingKey", "")) not in set(valid_ids)
            ]
    ...
    def get_content_changes_since(self, last_update: datetime | None = None):
        ...
        try:
            ...
            logger.info(
                f"Jellyfin incremental: Found {len(artists_to_return)} artists with {len(new_tracks)} new tracks"
            )
    ...
    def needs_update_by_age(self, artist, refresh_interval_days: int) -> bool:
        ...
        try:
            ...
            if not needs_update:
                logger.debug(
                    f"Skipping {artist.title}: updated {days_since_update} days ago (threshold: {refresh_interval_days})"
                )
    ...
    def get_album_tracks_as_echosync(self, album) -> list:
        ...
        """
        from .adapter import convert_jellyfin_track_to_echosync
        ...
        try:
            ...
            logger.debug(
                f"Getting {len(tracks)} tracks from Jellyfin album '{_safe_getattr(album, 'title', 'Unknown')}'"
            )
            ...
            if failed_count > 0:
                logger.warning(
                    f"⚠️ Jellyfin album '{_safe_getattr(album, 'title', 'Unknown')}': {len(tracks)} tracks, {len(echo_sync_tracks)} converted, {failed_count} failed"
                )
