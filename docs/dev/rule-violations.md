...
| Ungated File Move | `services/download_manager.py` | 1951 | `shutil.move(str(src), str(dest))` | Route through `Gatekeeper.authorize_and_execute` / `echosync_core.safe_move_file` |
...
| Ungated DB Deletion | `web/routes/system.py` | 944 | `os.remove(db_path)` | Route through `Gatekeeper.authorize_and_execute` |
| Direct SQLite Connection | `core/matching_engine/fingerprinting.py` | 312 | `sqlite3.connect(self.db_path, timeout=30.0)` | Refactor to use DatabaseGateway scoped ORM sessions (`session_scope()`) |
| Direct SQLite Connection | `core/matching_engine/fingerprinting.py` | 341 | `sqlite3.connect(self.db_path, timeout=30.0)` | Refactor to use DatabaseGateway scoped ORM sessions (`session_scope()`) |
| Direct SQLite Connection | `core/matching_engine/fingerprinting.py` | 375 | `sqlite3.connect(self.db_path, timeout=30.0)` | Refactor to use DatabaseGateway scoped ORM sessions (`session_scope()`) |
| Direct SQLite Connection | `core/matching_engine/fingerprinting.py` | 399 | `sqlite3.connect(self.db_path, timeout=30.0)` | Refactor to use DatabaseGateway scoped ORM sessions (`session_scope()`) |
