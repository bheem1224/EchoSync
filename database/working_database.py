...
class ReviewTask(WorkingBase):
    ...
    def detected_metadata(self, val: dict | None):
...
class VirtualTrackCache(WorkingBase):
    ...
    album_title: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
...
_working_db_instance: WorkingDatabase | None = None


def get_working_database(database_path: str | None = None) -> WorkingDatabase:
