...
down_revision: str = (
    "abcd12345678"  # Note: Assuming this based on the files seen, I will double check
)
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    for idx in [
        "idx_services_namespace",
        "idx_services_display_name",
        "idx_services_friendly_name",
    ]:
        if idx in indexes:
    ...
    with op.batch_alter_table("services", schema=None) as batch_op:
        ...
        if "absolute_install_path" not in columns:
            batch_op.add_column(
                sa.Column("absolute_install_path", sa.String(), nullable=True)
            )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("services", schema=None) as batch_op:
        if "namespace" not in columns:
            batch_op.add_column(
                sa.Column(
                    "namespace", sa.String(), nullable=False, server_default="legacy"
                )
            )
