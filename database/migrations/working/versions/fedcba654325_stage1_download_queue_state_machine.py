...
down_revision: Union[str, None] = 'ba366761575f'
...
def upgrade() -> None:
    ...
    table_info = connection.execute(sa.text("PRAGMA table_info(download_queue)")).fetchall()
    existing_columns = {row[1] for row in table_info}

    # Inspect existing indexes
    index_info = connection.execute(sa.text("PRAGMA index_list(download_queue)")).fetchall()
    ...
    with op.batch_alter_table('download_queue', schema=None) as batch_op:
        if 'intent' not in existing_columns:
            batch_op.add_column(
                sa.Column('intent', sa.String(32), nullable=False, server_default='MANUAL_OMNI')
            )
        ...
        if 'candidate_stack' not in existing_columns:
            batch_op.add_column(
                sa.Column('candidate_stack', sa.JSON(), nullable=False, server_default='[]')
            )
        if 'blacklisted_candidates' not in existing_columns:
            batch_op.add_column(
                sa.Column('blacklisted_candidates', sa.JSON(), nullable=False, server_default='[]')
            )
        ...
        if 'ix_download_queue_sync_id' not in existing_indexes:
            batch_op.create_index('ix_download_queue_sync_id', ['sync_id'], unique=False)

    # Normalize existing status values to uppercase
    connection.execute(sa.text("UPDATE download_queue SET status = UPPER(status) WHERE status IS NOT NULL"))
