...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "ALTER TABLE _alembic_tmp_audio_fingerprints RENAME TO audio_fingerprints;"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX ix_audio_fingerprints_chromaprint ON audio_fingerprints (chromaprint);"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX ix_audio_fingerprints_media_id ON audio_fingerprints (media_id);"
        )
    )
