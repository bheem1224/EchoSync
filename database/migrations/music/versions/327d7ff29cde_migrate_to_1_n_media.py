...
down_revision: Union[str, None] = 'eaf4b5d2df68'
...
def upgrade() -> None:
    ...
    tracks = connection.execute(sa.text("SELECT id, sync_id, file_path, file_format, bitrate, sample_rate, bit_depth, file_size_bytes, inode, mtime, added_at FROM tracks")).fetchall()

    for track in tracks:
        new_sync_id = generate_nanoid()
        connection.execute(sa.text("UPDATE tracks SET sync_id = :sid WHERE id = :tid"), {"sid": new_sync_id, "tid": track.id})
        ...
        if track.file_path:
            ...
            """), {
                "mid": new_media_id, "tid": track.id, "fp": track.file_path, "ff": track.file_format, "br": track.bitrate,
                "sr": track.sample_rate, "bd": track.bit_depth, "fsb": track.file_size_bytes, "ino": track.inode, "mtime": track.mtime, "add": track.added_at
            })
    ...
    connection.execute(sa.text("DELETE FROM external_identifiers WHERE plugin_source IN ('spotify', 'deezer', 'tidal', 'apple_music')"))
    ...
    with op.batch_alter_table('audio_fingerprints') as batch_op:
        ...
        batch_op.create_foreign_key('fk_audio_fp_media', 'local_media', ['media_id'], ['media_id'], ondelete='CASCADE')
    ...
    with op.batch_alter_table('external_identifiers') as batch_op:
        ...
        batch_op.drop_column('track_id')
        batch_op.alter_column('media_id', nullable=False)
        batch_op.create_foreign_key('fk_ext_id_media', 'local_media', ['media_id'], ['media_id'], ondelete='CASCADE')
