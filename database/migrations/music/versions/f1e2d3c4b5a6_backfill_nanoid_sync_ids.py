...
down_revision: str | None = "e11aada610d5"
        ...
        try:
            ...
            with working_db.engine.begin() as wconn:
                for old_sync_id, new_sync_id in old_to_new.items():
                    ...
                    wconn.execute(
                        sa.text(
                            "UPDATE user_ratings SET sync_id = :new_id WHERE sync_id = :old_id OR sync_id = :base_old"
                        ),
                        {
                            "new_id": new_sync_id,
                            "old_id": old_sync_id,
                    ...
                    wconn.execute(
                        sa.text(
                            "UPDATE user_track_states SET sync_id = :new_id WHERE sync_id = :old_id OR sync_id = :base_old"
                        ),
                        {
                            "new_id": new_sync_id,
                            "old_id": old_sync_id,
                    ...
                    wconn.execute(
                        sa.text(
                            "UPDATE download_queue SET sync_id = :new_id WHERE sync_id = :old_id OR sync_id = :base_old"
                        ),
                        {
                            "new_id": new_sync_id,
                            "old_id": old_sync_id,
