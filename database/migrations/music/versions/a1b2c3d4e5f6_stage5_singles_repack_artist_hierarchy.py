...
def upgrade() -> None:
    ...
    if "release_type" not in track_columns:
        with op.batch_alter_table("tracks", schema=None) as batch_op:
            batch_op.add_column(
                sa.Column(
                    "release_type", sa.String(), nullable=True, server_default="album"
                )
            )
            batch_op.create_index(
                batch_op.f("ix_tracks_release_type"), ["release_type"], unique=False
            )
    ...
    if "parent_artist_id" not in artist_columns:
        with op.batch_alter_table("artists", schema=None) as batch_op:
            batch_op.add_column(
                sa.Column("parent_artist_id", sa.Integer(), nullable=True)
            )
            batch_op.create_foreign_key(
                "fk_artists_parent_artist_id",
                "artists",
                ["parent_artist_id"],
                ["id"],
                ondelete="SET NULL",
            ...
            batch_op.create_index(
                batch_op.f("ix_artists_parent_artist_id"),
                ["parent_artist_id"],
                unique=False,
            )
