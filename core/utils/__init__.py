...
class PathMapper:
    ...
    def __init__(self, mappings: list[dict[str, str]] | dict[str, dict[str, str]]):
    ...
    def map_to_local(self, remote_path: str) -> str:
        ...
        try:
            ...
            plugin_path = hook_manager.apply_filters(
                "RESOLVE_STORAGE_PATH", None, remote_path=remote_path
            )
        ...
        for mapping in self.mappings:
            ...
            search_prefix = (
                remote_prefix.rstrip("/") if len(remote_prefix) > 1 else remote_prefix
            )

            is_match = False
            if (
                search_prefix == "/"
                or normalized_remote == search_prefix
                or normalized_remote.startswith(search_prefix + "/")
            ):
                is_match = True
