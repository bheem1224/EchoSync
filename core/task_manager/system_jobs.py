        ...
        try:
            ...
            if PluginRegistry.get_plugin_class(
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to run primary local media server update: {e}",
                        exc_info=True,
                    )
            else:
                logger.debug(
            ...
            try:
                ...
                for p_id in PluginRegistry.get_plugins_by_type(
                    ...
                    has_url = bool(
                        config_db.get_service_config(svc_id, "server_url")
                        or config_db.get_service_config(svc_id, "url")
                        or config_db.get_service_config(svc_id, "host")
                    )
...
def register_external_identifier_sync_job(
    ...
    def run_external_identifier_sync(plugin_source: str | None = None, **kwargs):
        try:
                ...
                try:
                    ...
                    with db.session_scope() as session:
                        ...
                        existing_plugin_item_ids = set(
                            r[0]
                            for r in session.query(ExternalIdentifier.plugin_item_id)
                            .filter(ExternalIdentifier.plugin_source == source_name)
                            .all()
                        )
                        existing_media_ids = set(
                            r[0]
                            for r in session.query(ExternalIdentifier.media_id)
                            .filter(ExternalIdentifier.plugin_source == source_name)
                            .all()
                        )
                        ...
                        logger.info(
                            f"External identifier sync for {source_name}: successfully synced {synced_count} identifiers."
                        )

                except Exception as e:
                    logger.error(
                        f"External identifier sync worker failed for {active_server}: {e}",
                        exc_info=True,
                    )
...
def register_media_server_scan_job(interval_seconds: int = 10800, enabled: bool = True):
    ...
    def run_media_server_scan(section: str | None = None, **kwargs):
        try:
...
def register_retroactive_metadata_enhancement_job(
            ...
            try:
                ...
                for p in plugins:
                    if p.get("_installed") and p.get("update_available"):
                        ...
                        c.execute(
                            "SELECT id FROM services WHERE plugin_id=? OR name=?",
                            (plugin_id, plugin_id),
                        )
                        row = c.fetchone()
                        if row:
                            c.execute(
                                "UPDATE services SET available_version = ? WHERE id = ?",
...
def cleanup_orphaned_plugin_databases():
    ...
    with db._get_connection() as conn:
        c = conn.cursor()
        c.execute(
            "SELECT id, plugin_id, absolute_install_path FROM services WHERE is_active = 1 AND absolute_install_path IS NOT NULL"
        )
        records = c.fetchall()
        for row in records:
            ...
            if absolute_path:
                ...
                if not exists and not is_in_plugins_dir:
                    logger.warning(
                        f"Sweeper detected missing plugin folder outside plugins dir for plugin_id {plugin_id}: {absolute_path}. Removing ghost registry entry."
                    )
