...
class HttpError(Exception):
    ...
    def __init__(
        self, message: str, status: int | None = None, response: Response | None = None
    ):
...
class RequestManager:
    ...
    def __init__(
        ...
        rate: RateLimitConfig | None = None,
    ):
        """
    ...
    def _load_rate_limit_from_config(self) -> RateLimitConfig:
        ...
        try:
            return (
                RateLimitConfig(requests_per_second=float(rps))
                if rps is not None
                else RateLimitConfig(requests_per_second=1.0)
            )
    ...
    def _apply_rate_limit(self, url: str):
        ...
        if not self.rate.requests_per_second or self.rate.requests_per_second <= 0:
            return
        GlobalRateLimiter.get_instance().wait_for_url(
            url, self.rate.requests_per_second
        )

    def _should_retry(
        self, resp: Response | None, exc: Exception | None, attempt: int
    ) -> bool:
