...
from database.music_database import ExternalIdentifier, TrackAudioFeatures, get_database
...
def calculate_user_vibe(user_id: str, days: int = 30) -> dict[str, float] | None:
    """
    ...
    try:
        ...
        plugin_vibe = hook_manager.apply_filters(
            "PROVIDE_VIBE_PROFILE", None, user_id=user_id, days=days
        )
    ...
    if not user_play_counts:
        logger.debug(
            f"No recent playback history found for user_id={user_id} in the last {days} days."
        )
        return None
    ...
    with music_db.session_scope() as session:
        ...
        identifiers = (
            ...
            .filter(ExternalIdentifier.provider_item_id.in_(provider_ids_list))
            .all()
        )
        ...
        if not pid_to_sync_id:
            logger.debug(
                f"Could not map any provider IDs to tracks for user_id={user_id}."
            )
            return None
        ...
        all_features = (
            session.query(TrackAudioFeatures)
            .filter(TrackAudioFeatures.sync_id.in_(unique_sync_ids))
            .all()
        )
    ...
    if total_weight == 0:
        logger.debug(
            f"Could not calculate vibe signature for user_id={user_id}. No valid audio features found for recent tracks."
        )
        return None
...
def calculate_vibe_distance(
    target_vibe: dict[str, float], track_features: TrackAudioFeatures
) -> float:
    """
