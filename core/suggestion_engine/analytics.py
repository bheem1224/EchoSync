        ...
        try:
            ...
            with working_db.session_scope() as session:
                if cls._is_empty(session):
                    logger.debug(
                        "No playback data available (PlaybackHistory is empty)."
                    )
                    return 0
            ...
            logger.error(
                f"Error getting listen count for {provider_item_id}: {e}", exc_info=True
            )
            return 0

    @classmethod
    def get_trending_provider_ids(
        cls, days: int = 7, limit: int = 50
    ) -> dict[str, int]:
        ...
        try:
            ...
            with working_db.session_scope() as session:
                ...
                results = (
                    ...
                    .filter(PlaybackHistory.listened_at >= cutoff_date)
                ...
                return {
                    str(row.provider_item_id): int(row.play_count) for row in results
                }
        except Exception as e:
    ...
    def get_stale_provider_ids(cls, inactive_days: int = 90) -> list[str]:
        ...
        try:
            ...
            with working_db.session_scope() as session:
                ...
                all_time_listened_subq = (
                    session.query(PlaybackHistory.provider_item_id)
                    .group_by(PlaybackHistory.provider_item_id)
                    .subquery()
                )
                ...
                # Find the difference
                stale_results = (
                    session.query(all_time_listened_subq.c.provider_item_id)
                    .outerjoin(
                        recently_listened_subq,
                        all_time_listened_subq.c.provider_item_id
                        == recently_listened_subq.c.provider_item_id,
                    )
                    .filter(recently_listened_subq.c.provider_item_id.is_(None))
                    .all()
                )
