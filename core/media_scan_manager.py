...
class MediaScanManager:
    ...
    def __init__(self, delay_seconds: int = 60):
        ...
        self._is_doing_periodic_updates = (
            False  # Track if we're in periodic update mode
        )
        ...
        logger.info(
            f"MediaScanManager initialized with {delay_seconds}s debounce delay"
        )
    ...
    def _get_active_media_client(self):
        ...
        try:
            ...
            logger.error(
                f"Active media server '{active_server}' is not connected or unavailable."
            )
            return None, None
    ...
    def request_scan(self, reason: str = "Download completed") -> None:
        ...
        with self.lock_context():
            if self._scan_in_progress:
                ...
                logger.info(
                    f"📡 Media scan in progress - queueing follow-up scan ({reason})"
                )
                return
            ...
            if self._timer:
                ...
                logger.info(
                    f"⏳ Media scan queued - will execute in {self.delay}s ({reason})"
                )
    ...
    def add_scan_completion_callback(self, callback):
        ...
        with self._lock:
            if callback not in self._scan_completion_callbacks:
                ...
                logger.info(
                    f"DEBUG: Added scan completion callback: {callback.__name__}"
                )
                logger.info(
                    f"DEBUG: Total callbacks registered: {len(self._scan_completion_callbacks)}"
                )
    ...
    def _execute_scan(self):
        ...
        with self._lock:
            if self._scan_in_progress:
                logger.warning(
                    "Scan already in progress - skipping duplicate execution"
                )
                return
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ {server_type.upper()} library scan initiated successfully"
                )
    ...
    def _start_periodic_updates(self):
        ...
        try:
            ...
            logger.info(
                f"🕒 Starting periodic database updates - will check/update every {self._periodic_update_interval // 60} minutes"
            )

            # Schedule first periodic update after 5 minutes
            self._periodic_update_timer = threading.Timer(
                self._periodic_update_interval, self._do_periodic_update
            )
        ...
        try:
            with self._lock:
                if not self._scan_in_progress:
                    logger.debug(
                        "Scan no longer in progress, stopping periodic updates"
                    )
                    return
                ...
                if (
                    ...
                    and (time.time() - self._scan_start_time) > self._max_scan_time
                ):
                    logger.warning(
                        f"Media scan timeout reached ({self._max_scan_time}s), stopping periodic updates"
                    )
            ...
            elapsed_time = (
                time.time() - self._scan_start_time if self._scan_start_time else 0
            )

            if server_type:
                logger.info(
                    f"🕒 PERIODIC UPDATE: After {elapsed_time // 60:.0f} minutes - {server_type.upper()} scanning: {is_scanning}"
                )
            else:
            ...
            if is_scanning:
                ...
                if server_type:
                    logger.info(
                        f"🔄 {server_type.upper()} still scanning - triggering database update"
                    )
                else:
                    logger.warning(
                        "Database update skipped due to missing server type."
                    )
                ...
                logger.info(
                    f"🕒 Scheduling next periodic update in {self._periodic_update_interval // 60} minutes"
                )
                self._periodic_update_timer = threading.Timer(
                    self._periodic_update_interval, self._do_periodic_update
                )
                ...
                if server_type:
                    logger.info(
                        f"✅ {server_type.upper()} scanning completed - doing final database update"
                    )
                else:
                    logger.warning(
                        "Final database update skipped due to missing server type."
                    )
    ...
    def _scan_completed(self):
        ...
        with self._lock:
            ...
            if not was_in_progress:
                logger.debug(
                    "Scan completion callback called but scan was not in progress"
                )
                return
    ...
    def _call_completion_callbacks(self):
        ...
        with self._lock:
            callbacks = (
                self._scan_completion_callbacks.copy()
        ...
        for callback in callbacks:
            try:
                ...
                callback()
                logger.info(
                    f"DEBUG: Callback {callback.__name__} completed successfully"
                )
            except Exception as e:
                logger.error(
                    f"Error in scan completion callback {callback.__name__}: {e}"
                )
    ...
    def update_local_database(self):
        ...
        if not client or not server_type:
            logger.error(
                "Cannot update local database: No active media server available."
            )
            return

        try:
            logger.info(
                f"Updating local database with files from {server_type.upper()}..."
            )
