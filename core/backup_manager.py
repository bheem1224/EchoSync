...
class BackupManager:
    ...
    def _sqlite_backup(self, source_path: Path, target_path: Path):
        ...
        try:
            ...
            dst_conn.close()
            logger.info(
                f"SQLite backup successful: {source_path.name} -> {target_path.name}"
            )
    ...
    def create_backup(self) -> str:
        ...
        try:
            ...
            self._sqlite_backup(
                get_working_database().database_path, staging_dir / "working.db"
            )
            self._sqlite_backup(
                config_manager.media_db_path, staging_dir / "music_library.db"
            )
            ...
            for name, entry in PluginRegistry.get_all().items():
                ...
                plugins_snapshot[name] = {
                    ...
                    "channel": config_manager.get_plugin_channel(
                        str(name).split(".")[-1]
                    ),
                }
    ...
    def restore_backup(self, zip_path: Path):
        ...
        try:
                ...
                for zi in zipf.infolist():
                    ...
                    try:
                        if not target_path.is_relative_to(resolved_staging):
                            raise ValueError(
                                f"Malicious path in backup archive: {zi.filename}"
                            )
            ...
            if (staging_dir / "music_library.db").exists():
                shutil.copy2(
                    staging_dir / "music_library.db", config_manager.media_db_path
                )
    ...
    def _sync_plugins_to_snapshot(self, snapshot: dict[str, Any]):
        ...
        for plugin_id, metadata in snapshot.items():
            ...
            if plugin_id in store_map:
                ...
                logger.info(
                    f"Syncing plugin {plugin_id} to {target_version} ({channel})"
                )
                plugin_store.download_plugin(plugin_info, channel=channel)
            else:
                logger.warning(
                    f"Plugin {plugin_id} from backup not found in current store repositories."
                )
