<script>
  ...
  async function checkActiveServer() {
    try {
      ...
      isActive = response.data?.active_server === 'jellyfin';
    } catch (error) {
  ...
  async function activateServer() {
    try {
      activating = true;
      await apiClient.post('/media-server/activate', { server: 'jellyfin' });
...
<section class="jellyfin-card card">
      ...
      <div class="form-group">
        <label>
          ...
          <input
            ...
            class="input"
          />
          <span class="help-text">Enter your Jellyfin server URL (include port, typically :8096)</span>
        </label>
