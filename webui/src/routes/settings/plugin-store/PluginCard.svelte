...
<script>
  ...
  $: canInstallOrUpdate =
    !isInstalled || (useBeta ? hasBetaUpdate : hasStableUpdate);

  // Action Handlers
  function handleMainAction() {
    console.log(
      `[PluginCard] handleMainAction for ${plugin.id}. Channel: ${installedChannel}, StableUpdate: ${hasStableUpdate}, BetaUpdate: ${hasBetaUpdate}`,
    );
    if (downloading) return;

    const isBetaTarget =
      (targetVersion && targetVersion.includes("-beta")) ||
      (useBeta && hasBetaUpdate) ||
      (!isInstalled && useBeta);
    const chosenChannel = isBetaTarget ? "beta" : "stable";
    const chosenVersion = isBetaTarget ? latestBeta : latestRelease;

    console.log(
      `[PluginCard] Dispatching ${chosenChannel} action dispatch for ${plugin.id} (version: ${chosenVersion})`,
    );
    dispatch("install", {
      ...plugin,
      channel: chosenChannel,
      version: chosenVersion,
      target_version: chosenVersion,
  ...
  function handleSwitch(targetChannel) {
    console.log(
      `[PluginCard] handleSwitch to ${targetChannel} for ${plugin.id}`,
    );
    showDropdown = false;
    const isRollback =
      isInstalled && targetChannel === "release" && installedChannel === "beta";
    const normChannel = targetChannel === "beta" ? "beta" : "stable";
    const chosenVersion = targetChannel === "beta" ? latestBeta : latestRelease;
    dispatch("install", {
      ...plugin,
      channel: normChannel,
      version: chosenVersion,
  ...
  async function saveChannelPreference() {
    ...
    try {
      await apiClient.post(`/system/plugins/${plugin.id}/beta-opt`, {
        beta_opt_in: val,
      });
      ...
      alert(
        `Failed to save channel preference: ${err.response?.data?.error || err.message}`,
      );
    }
  }
  ...
  async function handleUndoUpdate() {
    if (
      confirm(
        "Are you sure you want to restore the previous stable version? Your current beta settings and state will be replaced by the snapshot taken before the update.",
      )
    ) {
      try {
        ...
        const errorMsg =
          ...
          "Failed to restore stable version. The snapshot might have expired.";
  ...
  async function handleRevertToStable() {
    try {
      ...
      alert(
        `Failed to revert to stable: ${err.response?.data?.error || err.message}`,
      );
    }
  }
  ...
  async function handleOptInBeta() {
    try {
      ...
      if (
        confirm(
          "Successfully opted-in to Beta. Would you like to check and download the Beta update now?",
        )
      ) {
        dispatch("install", {
          ...plugin,
      ...
      alert(
        `Failed to opt-in to beta: ${err.response?.data?.error || err.message}`,
      );
    }
  }
  ...
  async function handleClearLocalBeta() {
    try {
      ...
      alert(
        `Failed to reset channel: ${err.response?.data?.error || err.message}`,
      );
    }
  }
...
<div
  class="group relative flex flex-col p-5 bg-black/40 border border-white/10 rounded-2xl hover:border-blue-500/50 transition-all duration-300 shadow-xl backdrop-blur-md overflow-visible"
>
  <!-- Plugin Header -->
  <div class="flex items-start gap-4 mb-3">
    <div
      class="w-12 h-12 flex items-center justify-center bg-blue-500/10 text-blue-400 rounded-xl border border-blue-500/20"
    >
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center justify-between gap-2">
        <div
          class="flex items-center gap-1.5 flex-nowrap shrink-0 overflow-visible"
        >
          <h3
            class="text-lg font-bold text-white truncate max-w-[150px] lg:max-w-[200px]"
            title={plugin.name}
          >
            {plugin.name}
          </h3>
          {#if plugin.verified_source === "official"}
            <span
              class="px-1.5 py-0.5 bg-green-500/20 text-green-400 text-[10px] font-bold uppercase tracking-wider rounded border border-green-500/30 shrink-0 whitespace-nowrap"
              >Official</span
            >
          {:else if plugin._source_repo}
            <span
              class="px-1.5 py-0.5 bg-blue-500/20 text-blue-400 text-[10px] font-bold uppercase tracking-wider rounded border border-blue-500/30 shrink-0 truncate max-w-[120px] whitespace-nowrap"
              title={plugin._source_repo}
            >
        ...
        {#if isInstalled}
          <div class="relative">
            <button
              ...
              on:click={() =>
                (openMenuId = openMenuId === plugin.id ? null : plugin.id)}
            >
              ⋮
            </button>
            {#if openMenuId === plugin.id}
              <div
                class="absolute right-0 top-8 w-44 bg-slate-900 border border-white/10 shadow-2xl rounded-xl z-[110] overflow-hidden py-1"
              >
                <button
  ...
  {#if plugin.archive_expiry_date && getRemainingHours(plugin.archive_expiry_date) > 0}
    <div
      class="mb-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg flex items-center justify-between"
      transition:slide
    >
      <div class="pr-2">
        <h4 class="text-yellow-400 font-bold text-sm">Grace Period Active</h4>
        <p class="text-gray-300 text-[10px] mt-1 leading-tight">
          Your previous data snapshot expires in {getRemainingHours(
            plugin.archive_expiry_date,
          )} hours.
        </p>
      </div>
      <button
  ...
  >
    <div class="flex flex-wrap gap-2">
      <div
        class="flex items-center gap-1 px-2 py-1 bg-white/5 rounded-md border border-white/10"
        title="Version"
      >
        <span
      ...
      {#if isInstalled && installedChannel === "beta"}
        <span
          class="px-2 py-1 bg-purple-500/20 text-purple-400 text-[10px] font-bold uppercase rounded border border-purple-500/30"
          >Beta Track</span
        >
      {/if}
    ...
    <div class="relative flex items-stretch" use:clickOutside>
      {#if !globalBetaEnabled}
        ...
        <button
          class="px-4 py-2 {typeof downloading === 'string' &&
          downloading.startsWith('error-')
            ? 'bg-red-600 hover:bg-red-500 text-white'
            : 'bg-blue-600 hover:bg-blue-500 text-white disabled:bg-white/5 disabled:text-slate-500 disabled:border-white/10'} text-xs font-bold rounded-lg transition-all border-none flex items-center gap-2 cursor-pointer"
      ...
      {:else}
        ...
        <div
          class="flex rounded-lg overflow-hidden shadow-lg shadow-blue-900/20"
        >
          <!-- Main Action -->
          <button
            class="pl-4 pr-3 py-2 {typeof downloading === 'string' &&
            downloading.startsWith('error-')
              ? 'bg-red-600 hover:bg-red-500 text-white'
              : 'bg-blue-600 hover:bg-blue-500 text-white disabled:bg-slate-800 disabled:text-slate-500'} text-xs font-bold border-none transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer"
            disabled={downloading || !canInstallOrUpdate}
            on:click={handleMainAction}
          >
            {#if typeof downloading === "string" && downloading.startsWith("error-")}
          ...
          >
            <span
              class="inline-block transition-transform duration-200 {showDropdown
                ? 'rotate-180'
                : ''}">▼</span
            >
          </button>
...
{#if showChannelModal}
  <div
    class="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4"
  >
    <div
      class="w-full max-w-sm bg-slate-900/95 border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl relative"
    >
      <button
      ...
      <p class="text-xs text-slate-400 mb-4">
        Configure the active update and loading channel for <span
          class="text-white font-semibold">{plugin.name}</span
        >.
      </p>

      <div class="mb-5">
        <label
          class="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-2"
          >Channel Preference</label
        >
        <select
        ...
        >
          ...
          <option value="global"
            >⚙️ Use Global Settings ({globalBetaEnabled
              ? "Beta"
              : "Stable"})</option
          >
        </select>
