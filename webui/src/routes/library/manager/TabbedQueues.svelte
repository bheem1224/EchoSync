<script>
  ...
  function intentLabel(item) {
    const key =
      item.intent_type || item.type || item.action_needed || "SUGGESTION";
...
<div class="flex flex-col h-full min-h-0">
  ...
  <div class="flex gap-8 border-b border-glass-border mb-6 flex-shrink-0">
    ...
    <button
      id="tab-suggestions"
      class="pb-4 text-sm font-bold tracking-tight transition-all relative {activeTab ===
      'suggestions'
        ? 'text-primary'
    ...
    >
      ...
      {#if data.suggestions.length > 0}
        <span
          class="ml-2 px-1.5 py-0.5 rounded-full bg-primary/20 text-[10px] text-primary"
          >{data.suggestions.length}</span
        >
      {/if}
    ...
    >
      ...
      {#if data.pending.length > 0}
        <span
          class="ml-2 px-1.5 py-0.5 rounded-full bg-rose-500/20 text-[10px] text-rose-400"
  ...
  <div class="flex-grow min-h-0 overflow-y-auto custom-scrollbar pr-2">
    ...
    {#if activeTab === "suggestions"}
      {#if loading.suggestions}
        <div class="py-16 text-center text-muted italic">
          Loading suggestions…
        </div>
      {:else if data.suggestions.length === 0}
        <div class="py-16 flex flex-col items-center gap-3 text-muted">
          ...
          <p class="text-sm">
            No pending suggestions. The library looks clean.
          </p>
        </div>
      {:else}
        <div class="flex flex-col gap-3">
          {#each data.suggestions as item, index (item.id ?? (item.sync_id ? `${item.sync_id}_${item.type || item.intent_type || index}` : index))}
            <div
            ...
            >
              <div class="flex flex-col gap-1.5 min-w-0 mr-4">
                <div class="flex items-center gap-2 flex-wrap">
                  <span
                    class="text-[10px] uppercase font-black px-2 py-0.5 rounded-md border {intentBadgeClass(
                      item,
                    )}"
                  >
                    {intentLabel(item)}
                  ...
                  <span
                    class="text-sm font-bold text-white truncate max-w-[360px]"
              ...
              <div class="flex gap-2 flex-shrink-0">
                <button
                  ...
                  on:click={() =>
                    handleApproveSuggestion(item.sync_id, item.title)}
                >
                  APPROVE
                </button>
                <button
    ...
    {:else if activeTab === "pending"}
      ...
      {:else if data.pending.length === 0}
        <div class="py-16 flex flex-col items-center gap-3 text-muted">
          ...
          <p class="text-sm">
            No pending lifecycle actions. Everything is up to date.
          </p>
        </div>
      {:else}
        <div class="flex flex-col gap-3">
          {#each data.pending as item, index (item.sync_id ? `${item.sync_id}_${item.action_needed || index}` : index)}
            <div
              animate:flip={{ duration: 200 }}
            ...
            >
              <div class="flex flex-col gap-1.5 min-w-0 mr-4">
                <div class="flex items-center gap-2 flex-wrap">
                  <span
                    class="text-[10px] uppercase font-black px-2 py-0.5 rounded-md border {actionBadgeClass(
                      item.action_needed,
                    )}"
                  >
                  ...
                  <span
                    class="text-sm font-bold text-white truncate max-w-[320px]"
                    >{item.title || item.sync_id}</span
                  >
                </div>
                <div class="text-[11px] text-muted flex items-center gap-2">
                  {#if item.artist}<span>{item.artist}</span><span
                      class="w-1 h-1 rounded-full bg-muted/40"
                    ></span>{/if}
                  <span class="text-primary/80">
                    {item.days_in_queue != null
                      ? `${item.days_in_queue}d in queue`
                      : "Queued"}
                  </span>
                  {#if item.admin_exempt_deletion}
                    <span
                      class="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 text-[9px] uppercase font-bold border border-amber-500/30"
                      >Exempt</span
                    >
                  {/if}
                </div>
