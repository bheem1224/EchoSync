<script>
  ...
  async function loadSettings() {
    ...
    } catch (e) { console.error('Failed to load manager settings', e); }
  }
  ...
  async function loadProfiles() {
    ...
    } catch (e) { console.error('Failed to load quality profiles', e); }
  }
...
<div class="h-full w-full flex flex-col text-white p-6 md:p-10 gap-6 overflow-hidden">
  ...
  <header class="flex justify-between items-center flex-shrink-0">
    ...
    <div class="flex items-center gap-3">
      ...
      >
        <svg class="w-5 h-5 text-muted transition-transform duration-300 {settingsOpen ? 'rotate-90' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {#if settingsOpen}
    <div class="flex-shrink-0 bg-surface/30 backdrop-blur-sm border border-glass-border rounded-2xl p-6 animate-in">
      <div class="flex items-center justify-between mb-5">
        <h2 class="text-base font-bold text-white flex items-center gap-2">
          <span class="text-primary">⚙</span> Manager Settings
        </h2>
        <button class="text-[11px] text-muted hover:text-white transition-colors" on:click={() => settingsOpen = false}>
          Close ✕
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="flex flex-col gap-5">
          <div class="flex items-center justify-between">
            ...
            <label class="switch">
              <input type="checkbox" id="mm-enable-toggle" bind:checked={settings.enabled} />
        ...
        <div class="flex flex-col gap-5">
          <div class="flex flex-col gap-2">
            <label for="quality-profile-select" class="text-sm font-bold text-white">Quality Profile Enforcer</label>
            <select
        ...
        <div class="flex flex-col gap-5">
          {#if settings.enabled}
            <div class="flex flex-col gap-2">
              <label for="automation-level-select" class="text-sm font-bold text-white">Automation Level</label>
              <select
              ...
              <p class="text-[11px] text-muted leading-relaxed">{levelDescriptions[settings.automation_level]}</p>
            </div>
          {/if}

          <div class="flex items-start gap-2 bg-black/20 border border-glass-border rounded-lg p-3">
            ...
            <div class="flex flex-col gap-1">
              ...
              <p class="text-[11px] text-muted leading-relaxed">
                If a user rates a track 1 or 2 stars, EchoSync creates a <strong class="text-white/70">User Delete Request</strong>.
  ...
  <main class="flex-grow min-h-0 bg-surface/20 backdrop-blur-sm border border-glass-border rounded-[2rem] p-8 shadow-2xl overflow-hidden flex flex-col">
    <TabbedQueues />
  </main>
...
<style>
  ...
  .slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(255,255,255,0.08); border-radius: 24px; transition: 0.3s; border: 1px solid rgba(255,255,255,0.1); }
  .slider:before { position: absolute; content: ""; height: 18px; width: 18px; left: 2px; bottom: 2px; background-color: rgba(255,255,255,0.5); border-radius: 50%; transition: 0.3s; }
