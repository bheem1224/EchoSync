...
<div class="track-row group hover:bg-white/5 rounded-md px-3 py-2 grid grid-cols-[40px_1fr_60px_auto] items-center gap-2 transition-colors">
    ...
    <div class="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
            class="p-1.5 rounded-full hover:bg-blue-500/20 text-blue-400 transition-colors"
        ...
        <div class="relative menu-container">
            <button
                class="p-1.5 rounded-full hover:bg-gray-700 text-gray-400 hover:text-white transition-colors"
            ...
            {#if showMenu}
                <div class="absolute right-0 top-full mt-1 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-50 overflow-hidden text-sm">
                    <button class="w-full text-left px-4 py-2 hover:bg-gray-700 text-white flex items-center gap-2" on:click={() => handleAction('metadata')}>
                        <span>🔍</span> Fetch Metadata
                    </button>
                    <button class="w-full text-left px-4 py-2 hover:bg-gray-700 text-blue-300 flex items-center gap-2" on:click={() => handleAction('upgrade')}>
                        <span>⬆️</span> Force Upgrade
                    </button>
                    <div class="border-t border-gray-700 my-1"></div>
                    <button class="w-full text-left px-4 py-2 hover:bg-red-900/50 text-red-400 flex items-center gap-2" on:click={() => handleAction('delete')}>
                        <span>🗑</span> Delete
                    </button>
                    <button class="w-full text-left px-4 py-2 hover:bg-red-900/50 text-red-500 flex items-center gap-2" on:click={() => handleAction('force_delete')}>
