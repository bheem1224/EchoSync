  ...
  function connectStream() {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        processesByCategory = processes.reduce(
          (acc, p) => {
    ...
    <div class="header-right">
      {#if lastFetched}
        <span class="last-fetched"
          >Updated {lastFetched.toLocaleTimeString()}</span
        >
      {/if}
      <button
        class="refresh-btn"
              ...
              >
                ...
                <td class="td-task">
                  ...
                  {#if proc.wasm_instance_id}
                    <div class="wasm-id">
                      WASM: {proc.wasm_instance_id.substring(0, 8)}
                    </div>
                  {/if}
                </td>
                ...
                <td>
                  <span
                    class="type-badge {ownerTypeBadgeClass(proc.owner_type)}"
                  >
                ...
                <td class="td-action">
                  ...
                  {:else if state === "confirming"}
                    <div class="confirm-row">
                      ...
                      <button
                        class="btn-yes"
                        on:click={() => confirmKill(regId)}>Yes, kill</button
                      >
                      <button class="btn-no" on:click={() => cancelKill(regId)}
                        >Cancel</button
                      >
                    </div>
