  ...
  function connectStream() {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.error("Failed to parse SSE task stream data:", err);
      }
    };
    eventSource.onerror = () => {
      if (!health) {
        errorMsg = "Reconnecting live stream...";
      }
    };
  }
...
<div
  class="bg-gray-900/90 border border-gray-800 rounded-xl p-5 shadow-lg backdrop-blur-sm"
>
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center space-x-3">
      ...
      {#if health}
        ...
        <span
          class={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${color.bg} ${color.text}`}
        >
          <span class={`w-2 h-2 rounded-full ${color.dot} animate-pulse`}
          ></span>
  ...
  {#if errorMsg && !health}
    <div
      class="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-xs text-red-400 mb-3"
    >
      {errorMsg}
    </div>
  {/if}
  ...
  {#if queue}
    <div class="grid grid-cols-4 gap-3 my-4">
      <div
        class="bg-gray-800/50 border border-gray-700/50 rounded-lg p-3 text-center"
      >
      ...
      >
        <div class="text-2xl font-bold text-blue-400">
          {queue.stats.running || 0}
        </div>
        <div class="text-xs font-medium text-blue-300 mt-1">Running</div>
      </div>
      <div
        class="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 text-center"
      >
      ...
      <div
        class="bg-rose-500/10 border border-rose-500/20 rounded-lg p-3 text-center"
      >
        <div class="text-2xl font-bold text-rose-400">
          {queue.stats.blocked || 0}
        </div>
        <div class="text-xs font-medium text-rose-300 mt-1">
          Pending Blocked
        </div>
      </div>
    </div>
  ...
  {#if health && health.plugin_states && Object.keys(health.plugin_states).length > 0}
    <div class="mt-4 pt-3 border-t border-gray-800">
      <div
        class="text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wider"
      >
      ...
      <div class="flex flex-wrap gap-2">
        {#each Object.entries(health.plugin_states) as [pluginId, status]}
          ...
          <div
            class="flex items-center space-x-1.5 px-2.5 py-1 bg-gray-800/80 border border-gray-700/60 rounded-md text-xs"
          >
            <span class="font-medium text-gray-200">{pluginId}</span>
            <span
              class={`px-1.5 py-0.5 text-[10px] uppercase font-semibold rounded ${color.bg} ${color.text}`}
            >
              {status.state}
            </span>
          </div>
