...
class SyncAdapter:
    ...
    def trigger_sync(self, payload: dict) -> dict:
        ...
        if not sources:
            return {
                "accepted": False,
                "error": "At least one source provider is required.",
            }

        if mode == "provider-to-provider" and not provider_targets:
            return {
                "accepted": False,
                "error": "Select at least one provider target for provider-to-provider sync.",
            }

        if mode == "provider-to-library" and not library_targets:
            return {
                "accepted": False,
                "error": "Select at least one library target for provider-to-library sync.",
            }
