        ...
        with db.session_scope() as session:
            ...
            if not track and title and artist_name:
                track = (
                    session.query(Track)
                    .join(Artist)
                    .filter(Track.title.ilike(title), Artist.name.ilike(artist_name))
                    .first()
                )
            if track:
...
class SearchAdapter:
        ...
        async def async_worker():
            try:
                ...
                for plugin_id in PluginRegistry.list_plugins():
                    ...
                    is_downloader = isinstance(provider, DownloaderProvider) or getattr(
                    ...
                    try:
                        ...
                        if is_downloader:
                            try:
                                ...
                                profiles = (
                                    config_manager.get_quality_profiles()
                                    if hasattr(config_manager, "get_quality_profiles")
                                    else []
                                )
                        ...
                        for kind in kinds_to_search:
                            ...
                            if not is_downloader:
                                ...
                                if not getattr(
                                    caps.search,
                                    search_cap_keys.get(kind, "tracks"),
                                    False,
                                ):
                                    continue
                ...
                tasks = [
                    query_provider(provider, caps, plugin_id, is_downloader)
                    for provider, caps, plugin_id, is_downloader in search_providers
                ]
    ...
    async def federated_discovery(
        self,
        query: str,
        enabled_plugin_ids: list[int] | None = None,
    ...
    def route_result(self, item: dict, action: str, target: str | None = None) -> dict:
