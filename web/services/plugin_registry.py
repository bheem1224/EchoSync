from core.nexus_framework.plugin_loader import PluginRegistry as CorePluginRegistry
...
def list_plugins() -> list[dict]:
    ...
    try:
        with db._get_connection() as conn:
            c = conn.cursor()
            c.execute(
                "SELECT id, plugin_id, name, version, service_type, capabilities FROM services"
            )
            for row in c.fetchall():
                ...
                capabilities = {
                    ...
                    "supports_library_scan": caps_dict.get(
                    ...
                    "resolve_fingerprint": caps_dict.get(
                        "supports_fingerprinting", False
                    ),
                ...
                plugin_dict = {
                    "id": str(
                        plugin_id
                    ),  # Changed from name to integer ID! Cast to string to satisfy API contract
                    "plugin_id": plugin_id,
                    "name": db_name,
                    "display_name": db_name.replace("plugin.", "")
                    .replace("echosync.", "")
                    .title(),
        ...
        logging.getLogger(__name__).error(
            f"Failed to list plugins from DB: {e}", exc_info=True
        )
...
def get_plugins_for_capability(capability: str) -> list[dict]:
    ...
    for plugin in list_plugins():
        ...
        if caps.get("supports_playlists") != "NONE" or caps.get("search", {}).get(
            "tracks"
        ):
...
def get_plugin(plugin_name: str) -> dict | None:
