...
class ApproveSuggestionRequest(BaseModel):
    track: dict[str, Any]
    account_id: int
    playlist_name: str | None = "Suggestions for You"
...
def get_pending_suggestions(account_id: int):
    ...
    try:
        ...
        try:
            ...
            hook_params = hook_manager.apply_filters(
                "BEFORE_SUGGESTION_GENERATION",
            ...
            if user:
                ...
                with working_db.session_scope() as session:
                    ...
                    user_ratings = (
                        ...
                        .filter(UserRating.user_id == user.id, UserRating.rating >= 4.0)
...
def get_suggestion_audit(request: Request):
    ...
    try:
        ...
        return {"audit_history": audit_history, "count": len(audit_history)}
