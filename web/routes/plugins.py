...
def toggle_plugin(plugin_id: str, data: TogglePluginRequest | None = None):
    ...
    with config_db_connection() as conn:
        ...
        if plugin_id_int is not None:
            c.execute(
                "SELECT id, name, plugin_id, is_active FROM services WHERE id=? OR plugin_id=?",
                (plugin_id_int, plugin_id_int),
            )
        else:
            c.execute(
                "SELECT id, name, plugin_id, is_active FROM services WHERE lower(name)=lower(?)",
                (plugin_id_str,),
            )
        row = c.fetchone()
        if row:
    ...
    with config_db_connection() as conn:
        ...
        if db_id is not None:
            ...
            c.execute(
                ...
                (target_active, db_plugin_id),
            )
        elif db_name is not None:
            c.execute(
                "UPDATE services SET is_active=? WHERE lower(name)=lower(?)",
                (target_active, db_name),
            )
        conn.commit()
...
def get_plugin_accounts(plugin_id: str):
    ...
    try:
        ...
        return {
            ...
            "items": [],
            "total": 0,
            "error": "Failed to fetch plugin accounts",
        }
    ...
    schemas = {
        "spotify": [
            ...
            },
            {
                ...
                "label": "Redirect URI",
                "type": "text",
                "default": "http://127.0.0.1:8008/api/spotify/callback",
            },
        ],
        "plex": [
            {
...
def _enrich_provider_capabilities(provider_dict, provider_name=None):
    ...
    try:
        from core.nexus_framework.plugin_loader import (
            get_plugin_capabilities as fetch_capabilities,
        )
    ...
    try:
        with config_db_connection() as conn:
            ...
            if str(plugin_id).isdigit():
                ...
                c.execute(
                    "SELECT absolute_install_path FROM services WHERE plugin_id = ? OR id = ?",
                    (pid_int, pid_int),
                )
            else:
                c.execute(
                    "SELECT absolute_install_path FROM services WHERE LOWER(name) = ?",
                    (str(plugin_id).lower(),),
                )
            row = c.fetchone()
            if row and row[0]:
                ...
                if service_id:
                    c.execute(
                        "SELECT absolute_install_path FROM services WHERE id = ?",
                        (service_id,),
                    )
                    row = c.fetchone()
                    if row and row[0]:
...
legacy_router = APIRouter(prefix="/api/plugins", tags=["Plugins Legacy"])
for _r in router.routes:
    if isinstance(_r, APIRoute):
        _subpath = _r.path[len("/api/v1/system/plugins") :]
