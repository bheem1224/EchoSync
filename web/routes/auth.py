...
def login(payload: LoginRequest, response: Response):
    try:
        ...
        if plugin_auth is not None and isinstance(plugin_auth, dict) and plugin_auth.get("authenticated") is True:
            ...
            token = generate_auth_token(payload.username or plugin_auth.get("user", "unknown"), csrf_token)
            
            response.set_cookie('echo_auth', token, httponly=True, secure=False, samesite='strict')
            response.set_cookie('echo_csrf', csrf_token, httponly=False, secure=False, samesite='strict')
            return plugin_auth
