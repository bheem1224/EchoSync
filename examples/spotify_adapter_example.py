...
class SpotifyAdapter:
        ...
        for spotify_track in spotify_tracks:
            ...
            if existing:
                ...
                track_id = self.create_stub(
                    ...
                    release_year=int(spotify_track["album"]["release_date"][:4])
                    if spotify_track["album"].get("release_date")
                    else None,
                )
    ...
    def enrich_from_spotify(self, track_id: str) -> Track:
        ...
        from services.match_service import MatchContext, MatchService
        ...
        best_match_obj = service.find_best_match(
        ...
        if best_match_obj and best_match_obj.confidence_score >= 70:
            ...
            best_match = next(
                (
                    r
                    for r in results
                    if r["name"] == best_match_obj.candidate_track.title
                ),
                results[0],
            )
            ...
            enriched = self.enrich_track(
                ...
                release_year=int(best_match["album"]["release_date"][:4])
                if best_match["album"].get("release_date")
                else None,
            )

            # Attach Spotify plugin_ref
...
def example_usage():
    ...
    for track_id in track_ids[:5]:
        ...
        print(
            f"{track.title} by {', '.join(track.artists)} (confidence: {track.confidence_score:.2f})"
        )
