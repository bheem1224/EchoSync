...
def zip_dir(dir_path: Path, zip_file_path: Path):
    ...
    exclude_prefixes = ["ui/", "src/", "target/", "releases/", "beta.zip"]
    with zipfile.ZipFile(zip_file_path, "w", zipfile.ZIP_DEFLATED) as z:
        for root, dirs, files in os.walk(dir_path):
            ...
            for pfx in exclude_prefixes:
                if str(rel_root).replace("\\", "/").startswith(
                    pfx
                ) or "__pycache__" in str(rel_root):
                    skip_dir = True
                    break
            if skip_dir:
                continue
            ...
            for file in files:
                ...
                skip_file = False
                for pfx in exclude_prefixes:
                    if str(rel_file).replace("\\", "/").startswith(
                        pfx
                    ) or "__pycache__" in str(rel_file):
                        skip_file = True
                        break
                if skip_file:
                    continue
