...
db_path = (
    "C:\\Users\\bheem\\Nextcloud2\\VS-Code-Projects\\EchoSync\\data\\music_library.db"
)
...
with db.session_scope() as session:
    ...
    _va_artist_ids_subq = session.query(Artist.id).filter(
        Artist.name.ilike("various artist%")
    )
    va_count = (
        session.query(Track).filter(Track.artist_id.in_(_va_artist_ids_subq)).count()
    )
    ...
    conditions.append(
        and_(
            Track.artist_id.in_(_va_artist_ids_subq),
            func.json_extract(Track.metadata_status, "$.artist_fixed_from_tags").is_(
                None
            ),
        )
    )
    ...
    for t in tracks:
        ...
        print(
            f"ID: {t.id} | Path: {t.file_path} | Local Path: {local_path_str} | Exists: {exists} | MBID: {t.musicbrainz_id} | Status: {t.metadata_status}"
        )
