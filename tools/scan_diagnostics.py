...
def run_diagnostics(target_dir: Path) -> None:
    print("=== EchoSync Scanner Diagnostic Run ===")
    ...
    for root, dirs, files in os.walk(target_dir):
        for file in files:
            ...
            if not artist:
                file_issues.append(
                    "Missing Artist Tag: Neither TPE1 (artist) nor TPE2 (album_artist) present"
                )
            ...
            if raw_track_num is not None and parsed_track_num is None:
                file_issues.append(
                    f"Invalid Track Number format: '{raw_track_num}' (could not coerce to int)"
                )
            ...
            if raw_disc_num is not None and parsed_disc_num is None:
                file_issues.append(
                    f"Invalid Disc Number format: '{raw_disc_num}' (could not coerce to int)"
                )
            ...
            try:
                ...
                if track_obj is None:
                    file_issues.append(
                        "create_echo_sync_track returned None (failed factory validation)"
                    )
            except Exception as exc:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", required=True, type=str, help="Target directory path to scan"
    )
