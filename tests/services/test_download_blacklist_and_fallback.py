            ...
            "directories": [
                {
                    "directory": "/music",
                    "files": [
                        {"filename": "track1.flac", "state": "Completed, Succeeded", "percentComplete": 100, "size": 1000},
                        {"filename": "track2.flac", "state": "Completed, Errored", "percentComplete": 10, "size": 1000},
                        {"filename": "track3.flac", "state": "Completed, Rejected", "percentComplete": 0, "size": 1000},
                        {"filename": "track4.flac", "state": "Queued, QueuedRemotely", "percentComplete": 0, "size": 1000},
                        {"filename": "track5.flac", "state": "InProgress", "percentComplete": 50, "size": 1000},
                    ]
                }
            ]
        }
            ...
            "directories": [
                {
                    "directory": "/music",
                    "files": [
                        {"id": "file-123", "filename": "song.flac"}
                    ]
                }
            ]
        }
...
async def test_ghost_transfer_fallback_and_blacklist(mock_work_db, mock_db):
    ...
    mock_provider._async_get_download_status = AsyncMock(return_value=None)  # Disappeared
    mock_provider._async_cancel_download = AsyncMock(return_value=True)

    with patch.object(dm, '_get_active_download_providers', return_value=[mock_provider]):
        with patch.object(dm, '_execute_waterfall_search_and_download', new_callable=AsyncMock) as mock_waterfall:
            await dm._check_active_downloads()

            # Verify cancellation was called
            mock_provider._async_cancel_download.assert_called_once_with("peer1|test.flac")
