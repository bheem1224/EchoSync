...
class MockMusicDatabase:
    def __init__(self):
        ...
        self.mock_result.mappings.return_value.all.side_effect = lambda: (
            self.cursor.fetchall.return_value
        )
...
def service(mock_db, mock_spotify_client):
    ...
    return PersonalizedPlaylistsService(
        database=mock_db, spotify_client=mock_spotify_client
    )
...
def test_get_parent_genre():
    ...
    assert (
        PersonalizedPlaylistsService.get_parent_genre("deep house")
        == "Electronic/Dance"
    )
...
def test_get_genre_playlist_parent_genre(
    service: PersonalizedPlaylistsService, mock_db
):
...
def test_build_custom_playlist(
    service: PersonalizedPlaylistsService, mock_db, mock_spotify_client
):
    ...
    mock_db.cursor.fetchall.return_value = [
        {
            "similar_artist_spotify_id": "similar1",
            "similar_artist_name": "Similar Artist 1",
        }
    ]
    ...
    mock_spotify_client.get_artist_albums.assert_called_once_with(
        "similar1", album_type="album,single", limit=10
    )
...
def test_get_discovery_tracks_by_category_uses_named_like_parameters(
    service: PersonalizedPlaylistsService, mock_db
):
