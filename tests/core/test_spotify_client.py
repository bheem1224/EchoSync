...
def test_is_configured_false(spotify_client):
    ...
    with (
        patch("core.file_handling.storage.get_storage_service") as mock_storage,
...
def test_existing_token_scope_does_not_invalidate(monkeypatch):
    ...
    with patch(
        ...
        return_value={"client_id": "fake", "client_secret": "fake"},
    ):
...
def test_cached_scope_used_even_if_oauth_invalidates(monkeypatch):
    ...
    with patch(
        ...
        with patch(
            "core.account_manager.AccountManager.get_account_token",
            return_value=limited_token,
        ):
...
def test_setup_client_prefers_account_creds(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda svc, aid: {
            "client_id": "acctid",
            "client_secret": "acctsec",
            "redirect_uri": "acct://cb",
        },
    )
...
def test_search_by_isrc_with_default_account(monkeypatch):
    ...
    with (
        patch(
            "core.settings.ConfigManager.get_service_credentials",
            return_value={"client_id": "id", "client_secret": "sec"},
        ),
        patch(
            "core.account_manager.AccountManager.list_accounts",
            return_value=[{"id": 42, "is_active": True}],
        ),
    ):
...
def test_client_credentials_fallback_when_no_user_accounts(monkeypatch):
    ...
    with (
        patch(
            "core.settings.config_manager.get_service_credentials",
