...
def _create_mock_plugin_folder(
    plugins_dir: Path, name: str, version: str, beta_version: str = None
) -> Path:
...
def test_fresh_installation(temp_plugins_env):
    ...
    with config_db._get_connection() as conn:
        c = conn.cursor()
        c.execute(
            "SELECT name, version, plugin_id, beta_opt_in, is_active FROM services WHERE plugin_id=3191146312"
        )
        row = c.fetchone()
...
def test_update_flow_success(temp_plugins_env):
    ...
    monkeypatch.setattr(
        plugin_store,
        "get_all_store_plugins",
        lambda: [{"id": "EchoSync.spotify", "name": "Spotify", "version": "2.4.3"}],
    )
...
def test_update_flow_syntax_error(temp_plugins_env):
    ...
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        ...
        zip_file.writestr(
            "__init__.py", "class ProviderClass:\n    def parse(self:\n        pass\n"
        )  # Syntax error
    ...
    monkeypatch.setattr(
        RequestManager, "get", lambda self, *args, **kwargs: mock_response
    )
    ...
    monkeypatch.setattr(
        plugin_store,
        "get_all_store_plugins",
        lambda: [{"id": "core.musicbrainz", "name": "Musicbrainz", "version": "1.0.1"}],
    )
...
def test_beta_opt_in_and_rollback(temp_plugins_env):
    ...
    plugin_path = _create_mock_plugin_folder(
        plugins_dir, "tidal", "2.4.0", beta_version="2.4.1-beta.2"
    )
    config_db.register_service(
        name="EchoSync.tidal",
        service_type="provider",
    ...
    with working_db.session_scope() as session:
        session.execute(
            text(
                "INSERT INTO plugin_state_kvs (plugin_id, key, value) VALUES ('3106502486', 'user_token', 'stable-token')"
            )
        )
    ...
    with working_db.session_scope() as session:
        res = session.execute(
            text(
                "SELECT value FROM plugin_state_kvs WHERE plugin_id = '3106502486@beta' AND key = 'user_token'"
            )
        ).fetchone()
    ...
    with config_db._get_connection() as conn:
        c = conn.cursor()
        c.execute(
            "SELECT beta_opt_in, absolute_install_path FROM services WHERE plugin_id = 3106502486"
        )
        row = c.fetchone()
    ...
    with working_db.session_scope() as session:
        res = session.execute(
            text("SELECT 1 FROM plugin_state_kvs WHERE plugin_id = '3106502486@beta'")
        ).fetchone()
        assert res is None
...
def test_uninstall_flow(temp_plugins_env):
    ...
    with working_db.session_scope() as session:
        session.execute(
            text(
                "INSERT INTO plugin_state_kvs (plugin_id, key, value) VALUES ('3191146312', 'cache_data', 'spotify-cached-tracks')"
            )
        )
    ...
    with working_db.session_scope() as session:
        res = session.execute(
            text("SELECT 1 FROM plugin_state_kvs WHERE plugin_id = '3191146312'")
        ).fetchone()
        assert res is None
...
def test_dynamic_module_tracking(temp_plugins_env):
    ...
    success = loader._load_plugin_package(
        123456789, absolute_install_path=str(plugin_path.resolve())
    )
    assert success is True
...
def test_plugin_id_reconciliation_and_orphaning(temp_plugins_env):
    ...
    spotify_crc = (
        binascii.crc32("EchoSync.spotify".lower().encode("utf-8")) & 0xFFFFFFFF
    )
    ...
    nonexistent_crc = (
        binascii.crc32("EchoSync.nonexistent".lower().encode("utf-8")) & 0xFFFFFFFF
    )
    config_db.register_service(
        name="Nonexistent",
        service_type="provider",
...
def test_get_or_create_service_id_filtering(temp_plugins_env):
    ...
    with config_db._get_connection() as conn:
        c = conn.cursor()
        c.execute(
            "SELECT name, absolute_install_path, plugin_id FROM services WHERE id = ?",
            (spotify_sid,),
        )
        row = c.fetchone()
        assert row["name"] == "EchoSync.spotify"
