...
def test_oauth_sidecar_plugins_callback_proxies_to_plugin_route(monkeypatch):
    ...
    assert (
        ...
        == "http://192.168.1.11:5000/api/plugins/2391116200/callback?code=testcode&state=1"
    )
...
def test_oauth_sidecar_tidal_callback_proxies_to_plugin_route(monkeypatch):
    ...
    assert (
        resp.headers["Location"]
        == "http://192.168.1.11:5000/api/plugins/3106502486/callback?code=testcode&state=1"
    )
