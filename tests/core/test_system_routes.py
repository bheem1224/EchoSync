...
def test_system_accounts_list(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "web.services.plugin_registry.list_plugins", lambda: mock_plugins
    )
    ...
    monkeypatch.setattr(
        "core.nexus_framework.plugin_loader.PluginRegistry", MockPluginRegistry
    )
...
def test_map_system_accounts(client, monkeypatch):
    ...
    # Make request
    payload = {"user_id": 100, "account_ids": [200, 300]}
