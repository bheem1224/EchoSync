...
def manager(mock_media_client):
    ...
    with patch.object(
        MediaScanManager,
        "_get_active_media_client",
        return_value=(mock_media_client, "plex"),
    ):
...
def test_follow_up_scan(manager: MediaScanManager):
    ...
    with patch.object(manager, "request_scan") as mock_request_scan:
        ...
        mock_request_scan.assert_called_once_with(
            "Follow-up scan for downloads during previous scan"
        )
...
def test_periodic_update_logic(manager: MediaScanManager, mock_media_client):
    ...
    with patch("threading.Timer") as mock_timer_class:
        ...
        assert (
            mock_timer_class.call_count == 2
        )  # Initial and the one inside _do_periodic_update
