...
class TestCapabilityFiltering:
    ...
    def test_has_capability_search(self):
        ...
        has_search = any(
            p.get("enabled") and "search.tracks" in p.get("search_capabilities", [])
            for p in self.providers
        )
    ...
    def test_has_capability_not_found(self):
        ...
        has_webdav = any(
            p.get("enabled") and "webdav" in p.get("special_capabilities", [])
            for p in self.providers
        )
...
class TestLibraryServers:
    ...
    def test_filter_by_scope_media_server(self):
        ...
        media_servers = [
            p
            for p in providers
            if p.get("enabled") and p.get("scope") == "media_server"
        ]
    ...
    def test_library_view_uses_media_servers_with_scan(self):
        ...
        library_capable_servers = [
            ...
            for p in providers
            if p.get("enabled")
            and p.get("scope") == "media_server"
            and p.get("supports_library_scan")
        ]
        assert len(library_capable_servers) == 3
        assert {p["name"] for p in library_capable_servers} == {
            "plex",
            "jellyfin",
            "navidrome",
        }
...
class TestProviderScopes:
    ...
    def test_filter_by_scope_metadata(self):
        ...
        metadata = [
            p for p in providers if p.get("enabled") and p.get("scope") == "metadata"
        ]
    ...
    def test_filter_by_scope_utility(self):
        ...
        utilities = [
            p for p in providers if p.get("enabled") and p.get("scope") == "utility"
        ]
...
class TestViewRendering:
    ...
    def test_search_view_renders_when_search_capable(self):
        ...
        has_search = any(
            p.get("enabled") and p.get("search_capabilities") for p in providers
        )
        assert has_search is True
    ...
    def test_library_view_renders_when_library_capable(self):
        ...
        providers = [{"name": "plex", "enabled": True, "supports_library_scan": True}]

        has_library = any(
            p.get("enabled") and p.get("supports_library_scan") for p in providers
        )
    ...
    def test_playlist_view_renders_when_playlist_capable(self):
        ...
        has_playlist = any(
            p.get("enabled") and p.get("playlist_support") for p in providers
        )
...
class TestProviderStateChanges:
    ...
    def test_provider_install_detection(self):
        ...
        initial_state = [{"name": "spotify", "enabled": True}]
    ...
    def test_provider_remove_detection(self):
        ...
        updated_state = [{"name": "spotify", "enabled": True}]
        ...
        initial_providers = [
            {
                "name": "spotify",
                "enabled": True,
                "search_capabilities": ["search.tracks"],
            }
        ]
...
class TestProviderAdaptationScenario:
    ...
    def test_scenario_install_search_provider(self):
        ...
        can_search = any(
            p.get("enabled") and p.get("search_capabilities") for p in initial_providers
        )
        ...
        providers_changed = set(p["name"] for p in initial_providers) != set(
            p["name"] for p in updated_providers
        )
        assert providers_changed is True, "Poller should detect new provider"

        # Step 4: View re-renders with new data
        can_search = any(
            p.get("enabled") and p.get("search_capabilities") for p in updated_providers
        )
    ...
    def test_scenario_disable_library_provider(self):
        ...
        initial_providers = [
            {"name": "plex", "enabled": True, "supports_library_scan": True}
        ]

        can_scan = any(
            p.get("enabled") and p.get("supports_library_scan")
        ...
        updated_providers = [
            {"name": "plex", "enabled": False, "supports_library_scan": True}
        ]
