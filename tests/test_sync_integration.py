...
def test_end_to_end_sync_workflow():
    ...
    assert sync_request["target_source"] in [
        "spotify",
        "tidal",
        "plex",
        "jellyfin",
    ...
    target_type = (
        "tier" if sync_request["target_source"] in tier_providers else "server"
    )
    ...
    assert all(
        "track_id" in m and "target_identifier" in m for m in ui_payload["matches"]
    )
