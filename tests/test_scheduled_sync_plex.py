...
def test_scheduled_sync_passes_target_user_id_to_plex(monkeypatch):
    ...
    monkeypatch.setattr(
        "web.routes.playlists.job_queue.register_job", fake_register_job
    )

    analysis_calls = []

    def fake_analyze_playlists_internal(
    ...
    monkeypatch.setattr(
        "web.routes.playlists._analyze_playlists_internal",
        fake_analyze_playlists_internal,
    )

    sync_calls = []

    def fake_sync_to_plex(
        payload,
        source,
        target,
        playlist_name,
