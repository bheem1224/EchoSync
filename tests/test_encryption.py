...
def run_test():
    ...
    assert retrieved_value == test_secret_value, (
        "FAIL: Retrieved value does not match original"
    )
    ...
    assert encrypted_value.startswith("enc:"), (
        "FAIL: Value in database is not prefixed with 'enc:'"
    )
    assert encrypted_value != test_secret_value, (
        "FAIL: Value in database is not encrypted."
    )
    ...
    assert retrieved_non_secret == non_secret_value, (
        "FAIL: Non-secret value did not match."
    )
    ...
    conn.close()
    db_config_data = json.loads(row[0])
    assert db_config_data["logging"]["level"] == non_secret_value, (
        "FAIL: Non-secret value was modified in DB."
    )
